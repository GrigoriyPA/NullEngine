...
SimpleCameraControl::SimpleCameraControl(
    ...
    : TimerClientBase<SimpleCameraControl>(timer)
    , EventsClientBase<SimpleCameraControl>(events)
    , settings_(settings)
    , camera_(camera)
    , window_(window)
...
void SimpleCameraControl::Update(FloatType delta_time) {
...
void SimpleCameraControl::OnEvent(const sf::Event& event) {
    ...
    camera_->Rotate(
        camera_->GetVertical(), static_cast<FloatType>(event.mouseMove.x - window_width_ / 2) * settings_.sensitivity
    );

    camera_->Rotate(
        camera_->GetHorizon(), static_cast<FloatType>(event.mouseMove.y - window_height_ / 2) * settings_.sensitivity
    );
