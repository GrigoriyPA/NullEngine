...
RotationAnimation::RotationAnimation(Vec3 axis, float rotation_speed)
    : TimedAnimator(std::bind(&RotationAnimation::OnRefresh, this, std::placeholders::_1))
    , axis_(axis.normalized())
    , rotation_speed_(rotation_speed) {
}

void RotationAnimation::OnRefresh(float delta_time) {
    const auto change = Rotation(axis_, rotation_speed_ * delta_time);

    UpdateTransform(change * GetCurrentTransform());
}

TranslationAnimation::TranslationAnimation(Vec3 start, Vec3 end, float speed)
...
void TranslationAnimation::OnRefresh(float delta_time) {
    time_ = Module(time_ + delta_time, period_);

    float ratio = 2.0 * time_ / period_;
    if (ratio > 1.0) {
        ratio = 2.0 - ratio;
    }
